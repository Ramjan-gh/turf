...
export function HomePage({ currentUser }: HomePageProps) {
  ...
  useEffect(() => {
    // Auto-scroll when slots are selected
    if (selectedSlots.length > 0 && personalInfoRef.current) {
      setTimeout(() => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="flex flex-col  md:flex-row w-full md:justify-center gap-8  p-4 rounded-xl drop-shadow-lg bg-gradient-to-br from-green-50 to-emerald-50 ">
              ...
              return (
                <motion.button
                  ...
                  className={`relative p-5 rounded-2xl transition-all shadow-md ${
                    booked
                      ? "bg-green-500 text-gray-400 cursor-not-allowed"
                      : selectedSlots.includes(time) // <- calculate selection here
                      ? " bg-gray-500 text-white shadow-lg ring-2 ring-purple-400 ring-offset-2"
                      : "bg-green-500 hover:from-orange-500 hover:to-red-600 text-white"
                  }`}
                >
                  {!booked && (
                    <motion.div
        ...
        {/* {showBookingModal && selectedSlots && (
          <BookingModal
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <p className="text-xl text-purple-900">
                ৳{selectedSlots.length * (selectedSportData?.pricePerHour || 0)}
              </p>
            </div>
          </div>
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            className="mt-3 w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white py-3 rounded-xl flex items-center justify-center gap-2 transition-all"
          >
            Continue to Booking
